<tr class="item-evento">
    ...
    <td class="center estadisticas-{{ $evento->id_evento }}">
        {!! Form::open(['url' => '/eventos/estadisticas/' . $evento->id_evento, 'method' => 'POST', 'id' => 'estadisticaEvento-' . $evento->id_evento]) !!}
            <i class='material-icons grey-text' style='cursor:pointer' onclick="$('#estadisticaEvento-{{ $evento->id_evento }}').submit();">insert_chart</i>
        {!! Form::close() !!}
    </td>
    <td class='edit center'>
        {!! Form::open(['url' => '/eventos/editar/' . $evento->id_evento, 'method' => 'POST', 'id' => 'editarEvento-' . $evento->id_evento]) !!}
            <i class='material-icons grey-text' style='cursor:pointer' onclick="$('#editarEvento-{{ $evento->id_evento }}').submit();">edit</i>
        {!! Form::close() !!}
    </td>
    <td class="center">
        <i class='material-icons delete grey-text' data-id="{{$evento->id_evento}}" style='cursor:pointer'>delete</i>
    </td>
</tr>
